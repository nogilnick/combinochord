...
public class HandModel {
    ...
    private final boolean[] fngrEnbl;
     ...
     */
    private boolean[] BMSToBoolArr(ENBL_FNGR fngrVal) {
    ...
    public boolean FindBarreFingering(FretPosition[] fp, FretPosition[] sp, boolean[] barArr) {    //TODO score fingering
            ...
            if (barArr[i]) {
                for (int j = sp[i].str + 1; j < fp.length; ++j) {
                    if (fp[j].frt == 0) {
                        ...
                        fp[j].note += sp[i].frt;
                        fp[j].frt = sp[i].frt;
                    } else if (fp[j].frt <= sp[i].frt) {    //New barred finger covers previously barred finger
                        isBarre = true;
                        fp[j].fingNum = sp[i].fingNum;
    ...
    public double FindBestFingering(FretPosition[] fp) {
        ...
        if (fp.length == 1) {
        ...
        } else if (fp.length == 2) {    //Try all 6 possible ways of choosing the 2 fingers without crossing
            int best = -1;
            ...
            fngrScr = bestScore;
        } else if (fp.length == 3) {    //Try all 4 ways of arranging the fingers without crossing
            int best = -1;
            double bestScore = Double.NEGATIVE_INFINITY;
            //1-2-3
            ...
            fngrScr = bestScore;
        } else if (fp.length == 4) {    //Use the only possible combination that doesn't cross fingers
            fp[0].fingNum = 0;
            fp[1].fingNum = 1;
            fp[2].fingNum = 2;
            fp[3].fingNum = 3;
     ...
     * @return The score
     */
    private double RateFingering(FretPosition[] fp) {
        double score = 0.0;
        switch (fp.length) {
            case 4:
